from fastapi import APIRouter, BackgroundTasks, Depends, Request, HTTPException, Response, status
...
def login(*, user_details: LoginDetails, db: Session = Depends(get_db),
          bg_tasks: BackgroundTasks, request: Request, response: Response):
...
def reset_password(email: EmailStr, bg_tasks: BackgroundTasks, requests: Request, db: Session = Depends(get_db)):
    ...
    if not done:
        raise HTTPException(status.HTTP_500_INTERNAL_SERVER_ERROR, "There was an error processing your request!")
    return {"msg": "Reset email sent"}


@auth_router.post("/reset-password-confirm/")
def rest_password_confirm(body: ResetPasswordDetails, bg_tasks: BackgroundTasks, request: Request,
                          db: Session = Depends(get_db)):
